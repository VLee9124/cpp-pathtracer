...
public:
  ...
  {
    // Return true if vector is close to zero in all axis
    auto s = 1e-8;
...
inline vec3 refract(const vec3& uv, const vec3& n, double etai_over_etat){
  auto cos_theta = std::fmin(dot(-uv, n), 1.0); // By vector dot
...
inline vec3 random_in_unit_disk(std::mt19937& rng) {
  while (true) {
    auto p = vec3(random_double(-1, 1, rng), random_double(-1, 1, rng), 0);
